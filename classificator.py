...
def classify_traffic_signs(train_set,validation_set, train_set_results, validation_set_results):
    ...
    for image in train_set:
        ...
        feature_vector = shape_extractor.extract_zernike(preprocess_image(image))

        # Then the HOG, our most important feature(s)
        #feature_vector = color_extractor.extract_hog(preprocess_image(image))
        feature_vector = append(feature_vector, color_extractor.extract_hog(preprocess_image(image)))

        # Then we extract the color features
        hue = color_extractor.extract_hue(cv2.imread(image))
        ...
        feature_vector = append(feature_vector,symbol_extractor.calculateDCT(preprocess_image(image)))
    ...
    clf = LogisticRegression(penalty='l2', dual=False, tol=0.0001, C=4, intercept_scaling=1,
    ...
    for im in train_set:
        ...
        validation_feature_vector = shape_extractor.extract_zernike(preprocess_image(im))

        # Extract validation_feature_vector
        #validation_feature_vector = color_extractor.extract_hog(color.rgb2gray(cv2.imread(im)))
        validation_feature_vector = append(validation_feature_vector, color_extractor.extract_hog(preprocess_image(im)))

        # Extract the same color features as the training phase
        hue = color_extractor.extract_hue(cv2.imread(im))
        ...
        validation_feature_vector = append(validation_feature_vector,symbol_extractor.calculateDCT(preprocess_image(im)))
