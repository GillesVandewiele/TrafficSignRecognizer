...
def build_cnn(input_var=None):
    ...
    network = lasagne.layers.Conv2DLayer(
            network, num_filters=32, filter_size=(3, 3), stride=1, pad='same',
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
    ...
    network = lasagne.layers.Conv2DLayer(
            network, num_filters=64, filter_size=(4, 4), stride=1, pad='same',
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
    ...
    """
    network = lasagne.layers.Conv2DLayer(
            network, num_filters=128, filter_size=(3, 3), stride=1, pad='same',
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
    ...
    network = lasagne.layers.Conv2DLayer(
            network, num_filters=128, filter_size=(4, 4), stride=1, pad='same',
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
...
def main(model='cnn', num_epochs=10):
    ...
    #TODO: gelabelde test images gebruiken
    ...
    for train, validation in kf:
        ...
        X_train = np.asarray([all_train_images[i%len(all_train_images)] for i in train])
        X_val = np.asarray([all_train_images[i%len(all_train_images)] for i in validation])
        y_train = np.asarray([all_results[i%len(all_train_images)] for i in train])
        y_val = np.asarray([all_results[i%len(all_train_images)] for i in validation])
        ...
        for epoch in range(num_epochs):
            ...
            val_acc = 0
            val_batches = 0
            for batch in iterate_minibatches(X_val, y_val, 100, shuffle=True):
                inputs, targets = batch
